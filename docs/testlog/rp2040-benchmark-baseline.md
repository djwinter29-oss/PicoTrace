...
| `18.0 MHz` | `0/3` | Downstream-limited failure point; failing trials hit `sink=ring` and `peak=256` |
| `18.5 MHz` | `0/3` | Also downstream-limited and no longer part of the current clean baseline |

Observed throughput across the verified MOSI pass points was about `7.6-8.4 Mb/s`.
...
| Highest clean `250 MHz` sample-hz | `10000000` |
| First unstable `250 MHz` sample-hz | `11000000` with `overruns=22 sticky=1` and `71/112` transactions |
| Clearly failing `250 MHz` sample-hz | `12000000` produced `22` overruns and only `90/112` transactions; `15000000` and above dropped much further |
...
- for the current `250 MHz` RP2040 bench, treat `10000000` as the practical upper clean point for the standard `i2cdetect` smoke workload.
...
| `250 MHz` | `17.5 MHz` | `18.0 MHz` (`0/3`) | 2026-07-21 rerun after I2C-path changes was downstream-limited (`sink=ring peak=256`) at `18.0 MHz` and above |
...
| --- | --- | --- | --- |
| `150 MHz` | `6.0 MHz` | `6.5 MHz` | `6.5 MHz` still stayed `running` but dropped to `100/112` with `overruns=6 sticky=1` |
| `200 MHz` | `7.5 MHz` | `8.0 MHz` | `8.0 MHz` stayed `running` but dropped to `94/112` with `overruns=9 sticky=1` |
| `225 MHz` | `9.0 MHz` | `9.5 MHz` | `9.5 MHz` stayed `running` but dropped to `107/112` with `overruns=3 sticky=1` |
| `250 MHz` | `10.0 MHz` | `11.0 MHz` | `11.0 MHz` stayed `running` but dropped to `71/112` with `overruns=22 sticky=1` |
...
for hz in 4000000 6000000 8000000 9000000 10000000 11000000 12000000; do
